...
import { getUserCategories, addUserCategory, updateUserCategory, deleteUserCategory } from '@/lib/supabase/categories';
import { CategoryFormData } from '@/types';
import { Button } from '../ui/button';
import { Input } from '../ui/input';
import { toast } from 'sonner';
import { X, Plus, Check, Trash2, Loader2, Pencil } from 'lucide-react';
...
const CategoryManagement = () => {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      setError(null);
      await addUserCategory({ name: newCategory });
      setShowAddForm(false);
      setNewCategory('');
      queryClient.invalidateQueries({ queryKey: ['user-categories'] });
    } catch (error: any) {
  ...
  const handleEditCategory = async () => {
    if (!editingCategoryId || !editingValue.trim()) return;

    try {
      setError(null);
      await updateUserCategory(editingCategoryId, { name: editingValue });
      setEditingCategoryId(null);
      setEditingValue('');
      queryClient.invalidateQueries({ queryKey: ['user-categories'] });
    } catch (error: any) {
    ...
    <div className="p-6 rounded-xl bg-black/20 backdrop-blur-md border border-white/10">
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
          {categories?.map(cat => (
            <li key={cat.id} className="flex items-center justify-between bg-white/5 px-4 py-2 rounded-lg">
              {editingCategoryId === cat.id ? (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="icon" onClick={handleEditCategory}>
      ...
      {error && (
        <div className="text-red-500 mt-3 text-sm font-medium">{error}</div>
      )}
    </div>
  );
};
